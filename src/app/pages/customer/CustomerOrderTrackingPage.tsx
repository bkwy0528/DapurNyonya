  ...
  const getStatusProgress = (status: string) => {
    switch (status) {
      case 'Pending Approval':
        return { step: -1, progress: 10 };
      case 'Order Received':
        return { step: 0, progress: 33 };
      case 'In Preparation':
        return { step: 1, progress: 66 };
      case 'Ready for Pickup':
      case 'Delivered':
        return { step: 2, progress: 100 };
      case 'Rejected':
              ...
              return (
                <Card key={order.id} className="overflow-hidden">
                  ...
                  <CardContent className="p-6 space-y-6">
                    ...
                    {/* Status Timeline - Only show for non-pending and non-rejected orders */}
                    ...
                    {/* Estimated Preparation Date */}
                    {order.status !== 'Delivered' && order.status !== 'Ready for Pickup' && order.status !== 'Pending Approval' && order.status !== 'Rejected' && (
                      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                        <div className="flex items-start space-x-3">
                          <Clock className="w-5 h-5 text-yellow-600 flex-shrink-0 mt-1" />
                          <div>
                            <p className="text-sm text-gray-700">Estimated Ready Date:</p>
                            <p className="font-semibold text-gray-900">
                              {new Date(new Date(order.deliveryDate).getTime() - 86400000).toLocaleDateString('en-US', {
                                month: 'long',
                                day: 'numeric',
                                year: 'numeric'
                              })}
