...
export default function IngredientEstimationPage({ user }: IngredientEstimationPageProps) {
  ...
  const calculateFromOrders = () => {
    ...
    setProductCounts([
      { name: 'Traditional Dumplings', count: counts['Traditional Dumplings'] || 0 },
      { name: 'Festive Cookies', count: counts['Festive Cookies'] || 0 },
      { name: 'Traditional Snacks', count: counts['Traditional Snacks'] || 0 }
    ]);

    calculateIngredients(
      counts['Traditional Dumplings'] || 0,
      counts['Festive Cookies'] || 0,
      counts['Traditional Snacks'] || 0
    );
  };
  ...
  const calculateIngredients = (dumplings: number, cookies: number, snacks: number) => {
    ...
    // Dumplings ingredients (per pack of 12)
    if (dumplings > 0) {
    ...
    // Cookies ingredients (per box of 20)
    if (cookies > 0) {
      estimatedIngredients.push(
    ...
    // Snacks ingredients (per 500g pack)
    if (snacks > 0) {
      estimatedIngredients.push(
  ...
  const handleManualCalculate = () => {
    calculateIngredients(
      productCounts[0].count,
      productCounts[1].count,
      productCounts[2].count
    );
  };
  ...
  const updateQuantity = (index: number, value: string) => {
    ...
    setIngredients(prev =>
      prev.map((ing, i) =>
        i === index ? { ...ing, quantity: numValue } : ing
      )
    );
  };
                ...
                <div className="flex items-center space-x-3">
                  {manualMode ? (
                    <Input
                      type="number"
                      value={product.count}
                      onChange={(e) => updateProductCount(index, e.target.value)}
