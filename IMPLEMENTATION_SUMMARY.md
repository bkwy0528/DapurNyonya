...
| Payment gateway | ToyyibPay **production** (`https://toyyibpay.com`) — FPX online banking and DuitNow QR / e-wallets |
...
The browser client (React SPA) renders all UI, performs advisory validation (form checks, date rules), and reads Firestore directly through a single data-access module (`src/app/utils/db.ts`). It also performs the admin's direct writes (products, ingredients, settings, batch configuration, order status updates).
...
1. Validates the amount and reads `TOYYIBPAY_SECRET_KEY` and `TOYYIBPAY_CATEGORY_CODE` from the functions environment (`functions/.env`, gitignored — these are now the **production** ToyyibPay account's credentials; never exposed to the client or stored in Firestore).
2. POSTs to `https://toyyibpay.com/index.php/api/createBill` with `billPaymentChannel: '0'`; when the client passes `paymentMethod: 'tng'` it additionally sets `enableDuitNowQR: '1'` and `chargeDuitNowQR: '0'` (DuitNow QR is an additive toggle on top of FPX in ToyyibPay's API — the FPX tab cannot be fully excluded, and DuitNow QR must also be activated for the category on ToyyibPay's dashboard to appear).
...
**`adminReinstateBatchOrder`** — callable; admin-only (same `ADMIN_EMAILS` check). Undoes a single `'expired'` pre-order so its customer can pay through the normal in-app flow instead of the admin taking payment out-of-band (which would never create an `orders` document — invisible to Order Management, ingredient deduction, and analytics). Requires `status === 'expired'`; in a transaction, re-checks the same conditions a fresh pre-order would face — batch not `'cancelled'`, today still before `prepStartDate`, and enough remaining capacity under `maxQuantity` — then sets `status: 'awaiting_payment'` with a freshly computed deadline (same `min(now + windowHours, prepStartDate)` clamp as `createBatchPreOrder`), restores the quantity to `productionBatches.currentQuantity`/`orderCount`, and sends the customer a push notification. Invoked from the "Reinstate" button in `ProductionCalendarPage` (`src/app/utils/submitOrder.ts`'s `adminReinstateBatchOrder` wrapper).
...
- **Other `localStorage` keys** — `pwa-install-dismissed`, `push-notif-nudge-dismissed`, `notif-last-seen:{userId}` (per-user "last seen" timestamp for the header notification bell's unread badge).
...
`src/app/utils/db.ts` is the single client data-access module — every client Firestore call goes through it. Domain helpers live in `src/app/utils/business.ts` (ordering rules, daily limits, date keys, order-number format), `src/app/utils/batchOrders.ts` (shared batch types + display helpers), `src/app/utils/ingredients.ts`, `src/app/utils/notifications.ts` (FCM permission/token/foreground handling, push register/unregister, local OS notifications), and `src/app/utils/notificationFeed.ts` (pure builders for the header bell's notification list).

---

## 5. Implemented Modules

**Authentication & profiles.** Email/password registration (client-side password policy: ≥8 characters, letters and digits), Google sign-in (auto-profile creation), email-based password reset. Customers edit name, phone, address, notes, and a cropped/compressed profile photo in `users/{uid}`; the admin has an equivalent editor writing to `adminProfile/main`. The profile page also carries the push-notification **on/off switch**: it reflects whether *this device* currently has a saved FCM subscription (`isPushRegistered` — deliberately not the browser's `Notification.permission`, which stays `granted` forever once given and can't be revoked from JS); turning it off invalidates the device token (`deleteToken`) and removes it from `users/{uid}.fcmTokens`, and re-enabling just registers a fresh token with no re-prompt.
...
- *Preparation lead time*: earliest selectable date = today + max `prepDays` across cart items — nothing else. (An earlier admin-configurable "extra advance-notice buffer", `orderLeadBufferDays`, was removed; the minimum lead time now comes solely from each product's own Preparation Days.) Calendar dates before that are disabled.
- *Open order windows*: general (non-batch-tracked) orders are only accepted on dates inside an admin-configured window — **stored in** `settings/business.openOrderRanges` (array of `{ start, end }`, `YYYY-MM-DD` inclusive), **configured on** the Production Schedule page's "General Order Availability" section, **enforced in** the checkout calendar's disabled-dates function and re-validated in `handlePlaceOrder`. Closed by default (no ranges = no selectable dates) until the admin adds at least one window; malformed or inverted entries are dropped by `normalizeOpenOrderRanges`.
- *No capacity check*: the former daily-capacity feature (remaining-slots display, full-date blocking against `dailyLimits`/`orderCounts`) was removed from checkout along with its admin UI — order volume was never the actual constraint. Only `submitOrder`'s vestigial log-only server re-check remains (see §3.2).
...
**Pre-order administration (`ProductionCalendarPage`, titled "Pre-Orders").** Now shows only the Production Calendar (the "General Order Availability" section it briefly hosted moved to the Production Schedule page): a calendar of batch production dates (amber = has batches, green = has a confirmed batch). Per date and batch-tracked product the admin can open a date with `minQuantity`/`maxQuantity` (writing the `productionBatches` doc directly — permitted by rules), edit min/max, close/reopen the date (`status` toggle), expand the pre-order list, cancel an individual not-yet-paid pre-order (`adminCancelBatchOrder` in `db.ts`: an atomic batched write setting the pre-order `'cancelled'` and decrementing the batch counters), and reinstate an individual `'expired'` pre-order via a "Reinstate" button next to its badge (calls the `adminReinstateBatchOrder` callable — see §3.2 — subject to a capacity and prep-cutoff re-check, unlike the client-side-only cancel).
...
**In-app notification bell (`NotificationBell.tsx` + `notificationFeed.ts`).** A bell in the global header for both roles, with an unread-count badge and a dropdown panel. The list is **derived entirely on the client from data each role already reads** — no `notifications` collection exists and no extra Firestore writes happen. The customer feed builds at most one item per order (latest status, using `statusUpdatedAt` when present) and per pre-order (placed / payment-window-open with deadline / expired / cancelled), pinning actionable payment-window items to the top; the admin feed lists *incoming* events only (new paid orders, new pre-orders, batches reaching minimum, expired payment windows) since the admin causes status changes themselves. Items older than 30 days are dropped and the list caps at 15. "Unread" is a per-user `localStorage` last-seen timestamp (`notif-last-seen:{userId}`): opening the panel clears the badge but keeps the in-panel highlights from a frozen copy taken at open time.

**Order management (admin).** Search (name/phone/order number), status filter, sort by placement or delivery date, expandable cards showing items, totals (legacy `deliveryCharge` shown when > 0; otherwise "Grab fee via WhatsApp" for delivery), payment method/transaction reference, addresses, and notes. Status advances forward-only via buttons: `Order Received` → `In Preparation` → (`Ready for Pickup` | `Out for Delivery`) → `Delivered` — direct Firestore field updates (`updateOrderFields`, which also stamps `statusUpdatedAt` so the customer's notification bell can say *when* the status changed), and that update is what fires the `onOrderStatusChange` push trigger. There is no approve/reject step and no manual order creation.

**Production schedule (admin, `ProductionSchedulePage`).** Reworked around date ranges. Hosts **"General Order Availability"**: the list of `openOrderRanges` windows (add via a range-mode calendar picker whose days with orders are highlighted amber; remove individually; clicking a saved window selects it as the active range). Below it, an **Orders** panel lists every non-rejected/non-cancelled order whose `deliveryDate` falls inside the selected range, grouped into one expandable dropdown per day with urgency badges (Overdue/Today/Tomorrow/Urgent/Upcoming) and suggested production stages by days remaining (prepare ingredients → start cooking → packaging day). The former `dailyLimits` management (per-date caps + `_default`) was removed together with the checkout capacity feature.
...
- `'collecting'` → `'confirmed'`: automatically, inside `createBatchPreOrder`'s transaction, the moment `currentQuantity` reaches `minQuantity`. Sets `confirmedAt` and `paymentDeadline` = `min(now + settings/business.batchPaymentWindowHours (admin-configurable in Settings; default 48, from DEFAULT_BATCH_PAYMENT_WINDOW_HOURS), prepStartDate)` — never later than the same cutoff that stops new pre-orders, so a late-tipping batch can't get a deadline landing after preparation was supposed to start.
...
- `'expired'` → `'awaiting_payment'`: admin-only, via the `adminReinstateBatchOrder` callable (the "Reinstate" button in `ProductionCalendarPage`). Re-checks batch not cancelled, today still before `prepStartDate`, and remaining `maxQuantity` capacity; sets a fresh clamped `paymentDeadline` and restores the quantity to the batch. This is the only path back into the paid flow for someone who missed their window — the alternative (admin collecting payment outside the app) never produces an `orders` document at all (see §14, decision 5).
...
- **Paid pre-orders never touch `dailyLimits`/`orderCounts`**: `submitBatchOrderPayment` performs no capacity check and no `orderCounts` increment; batch demand is bounded only by the batch's own `maxQuantity`. (Moot in practice now that the daily-capacity feature is removed, but the asymmetry with `submitOrder` — which still increments `orderCounts` — remains in code.)
...
| `orderCounts` | delivery date `YYYY-MM-DD` | `{ count }` orders booked that date | public (**no client reads it anymore** — the checkout capacity display was removed) | admin (increments still happen server-side in `submitOrder`'s transaction) |
...
| `dailyLimits` | date `YYYY-MM-DD`, or reserved `_default` | `{ limit }` per-date / default capacity cap — **legacy**: the feature's admin UI and checkout enforcement were removed; only `submitOrder`'s log-only re-check still reads it | public | admin (no UI writes it anymore) |
...
**Consistency mechanisms now in place:** the per-date `orderCounts` increment happens in the same transaction that creates the order (`submitOrder`); the daily `counters/orders-{YYMMDD}` sequence is incremented transactionally by both order-creating callables (so numbers are collision-free across flows); `productionBatches.currentQuantity`/`orderCount` are adjusted transactionally by `createBatchPreOrder`, `expireBatchPaymentsCore`, and `adminReinstateBatchOrder`, and by an atomic client batched write in `adminCancelBatchOrder`. Composite indexes exist (`firestore.indexes.json`) for the three server queries: `batchOrders(batchId, status)`, `batchOrders(status, paymentDeadline)`, `productionBatches(batchStatus, productionDate)`.
...
There is no named interface for the settings document; the authoritative shape is the union of the fields `AdminSettingsPage.tsx` and `ProductionSchedulePage.tsx` each save (both `merge: true`, fields listed in §5), with the open order ranges subtree typed as:
...
(A `settings/business.orderLeadBufferDays` field may linger in the live document from the removed lead-buffer feature; no code reads it anymore.)
...
- **Service worker**: Workbox-generated. Deviations from a standard setup: `registerType: 'prompt'` — a new deploy does **not** auto-activate; `main.tsx`'s `onNeedRefresh` shows a persistent toast with an "Update" action and only then swaps versions (no `skipWaiting` mid-session). `navigateFallbackDenylist: [/^\/__\//]` keeps Firebase's reserved paths (notably the `signInWithPopup` handler at `/__/auth/handler`) from being served the cached SPA shell. Precaching covers all build assets; runtime caching is StaleWhileRevalidate for scripts/styles/workers and CacheFirst for images (30 days) and fonts (1 year). The app shell therefore loads offline; live data still requires connectivity. **`firebase.json` hosting `headers`** force `Cache-Control: no-cache` on `/sw.js`, `/index.html`, and `/manifest.json` (everything else falls back to Firebase Hosting's default `max-age=3600`) — without this, browsers could serve an hour-old `sw.js` and never even check for a new deploy. `main.tsx`'s `onRegisteredSW` also calls `registration.update()` on `visibilitychange` (tab regaining focus) and every 30 minutes while the tab is open, instead of relying solely on cold-load checks.
- **Push service worker**: a second, hand-written `public/firebase-messaging-sw.js` is registered manually at scope `/firebase-cloud-messaging-push-scope/` (so it never fights Workbox for `/`), shows OS notifications for background FCM messages, and opens the notification's link on click. Foreground messages (tab focused) are caught in-page (`App.tsx` + `onMessage`) and shown as **the same OS-level notification** via the push registration's `showNotification` (`showLocalNotification` in `notifications.ts` — the plain `Notification` constructor is unsupported on Android Chrome), replacing the earlier easy-to-miss in-page toast, so a push looks identical whether or not the app is open.
...
- **Daily order capacity limits — REMOVED as a feature; server remnant only.** The admin UI for per-date/default caps (formerly on the Production Schedule page), checkout's remaining-slots display and full-date blocking, and the `dailyLimits`/`orderCounts` read helpers in `db.ts`/`business.ts` are all gone — order volume was never actually the constraint. Remnants: the `dailyLimits`/`orderCounts` security-rule blocks, existing Firestore documents, and `submitOrder`'s log-only transactional re-check (which still increments `orderCounts`).
- **Order lead-time buffer (`orderLeadBufferDays`) — REMOVED (no code remains).** Checkout's minimum lead time now comes solely from each product's own `prepDays`; the settings field may linger in the live document but nothing reads it.
...
|---|---|---|
| Online payment via ToyyibPay (FPX) | **Fully implemented** | **Production** ToyyibPay (`toyyibpay.com`) — switched from the sandbox pre-release; production credentials live in the gitignored `functions/.env` |
...
| Pre-order lifecycle: admin reinstatement of an expired pre-order | **Fully implemented** | `adminReinstateBatchOrder` callable, capacity- and prep-cutoff-checked, fresh clamped `paymentDeadline`; "Reinstate" button in `ProductionCalendarPage` |
| Push notifications | **Fully implemented** | Batch confirmed / expired / cancelled, order-status changes (`onOrderStatusChange`), admin test send; foreground messages shown as the same OS notification as background ones; per-device opt-in with a working off switch (token invalidated server-side) |
| In-app notification centre (header bell) | **Fully implemented (client-derived)** | Customer + admin feeds built purely from data each role already reads; nothing stored server-side; unread state is a per-user `localStorage` last-seen timestamp |
...
| Open order date windows (general products) | **Partially implemented — client-side only** | Stored in `settings/business.openOrderRanges`; admin-configured on the Production Schedule page; enforced in the checkout calendar and validation; `submitOrder` does not re-check it server-side; closed by default until a window is added |
| Order lead-time buffer (general products) | **Removed** | `orderLeadBufferDays` UI, computation, and helpers deleted; checkout's earliest date comes solely from the cart's max `prepDays` |
| Daily capacity limits | **Removed (server remnant only)** | Admin UI, checkout display/blocking, and client read helpers deleted; `submitOrder` still increments `orderCounts` and carries a vestigial log-only limit re-check |
...
5. **Paid work is never clawed back**: expiry/cancellation jobs only touch unpaid states, a confirmed batch never un-confirms, and an over-capacity paid order is accepted and logged rather than refused (the admin resolves rare overbooking manually; refusing captured money automatically was judged worse). The sanctioned way to honor a customer who missed their payment window is `adminReinstateBatchOrder`, not an out-of-band payment — the latter produces no `orders` document at all, silently exempting that sale from order tracking, ingredient deduction, and analytics.
...
7. **Ordering rules live in `settings/business`, not code**, so the admin changes the open date windows (e.g. a festive-season window) and the batch payment window without a release. Enforcement of the date rules is client-side only — accepted as sufficient for this trust model (the same customer sees the same calendar; the server still controls money and every order-creating write). The two other date levers were deliberately *simplified away*: the extra lead-time buffer (per-product `prepDays` alone decides the earliest date) and daily capacity caps (order volume was never the real constraint).
...
11. **Push is best-effort by construction**: a failed send never fails the business operation that triggered it, and dead tokens are pruned on the fly. The complementary in-app notification bell is **derived, not stored**: its feed is computed client-side from the orders/pre-orders each role already reads, so notification history costs zero extra writes and needs no `notifications` collection, at the price of per-device (not per-account) unread state.
